...
class ColetaListar extends Ordenacao
{
    ...
    private function dados($start, $length)
    {
        ...
        $sb->append("coleta_documento.numero as numero_documento,");
        $sb->append("(SELECT GROUP_CONCAT(coleta_documento.numero separator ', ') FROM coleta_documento where coleta_documento.id_coleta = ocorrencia_movimento.id_movimento) as documentos");
        $sb->append("FROM");
        ...
        $sb->append("LEFT JOIN coleta_documento");
        $sb->append("ON coleta_documento.id = ocorrencia_movimento.id_documento");
        
        $sb->append("WHERE");
        $sb->append($this->filtros());
        $sb->append("GROUP BY ocorrencia_movimento.id_ocorrencia");
        $sb->append("ORDER BY ocorrencia_movimento.data_ocorrencia DESC, ocorrencia_movimento.data_ocorrencia DESC, ocorrencia_movimento.id DESC");
    ...
    {
        $sb = new StringBuilder();
        $sb->append("ocorrencia_movimento.id_movimento = '{$_REQUEST['id_coleta']}' and ocorrencia_movimento.id_tipo_movimento = 3 and ocorrencia_movimento.status !=10");
        return $sb->toString();
    }
    ...
    public function detalhe()
    {
        ...
        $sb->append("ocorrencia_movimento.id_motorista,");
        $sb->append("coleta_documento.numero as numero_documento,");
        $sb->append("coleta_documento.id_coleta,");
        $sb->append("ocorrencia.nome as nome_ocorrencia,");
        $sb->append("ocorrencia_movimento.data_ocorrencia,");
        $sb->append("ocorrencia_movimento.hora_ocorrencia,");
        $sb->append("ocorrencia_movimento.id_manifesto,");
