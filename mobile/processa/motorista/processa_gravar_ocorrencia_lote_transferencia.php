...
try {
    ...
    LEFT JOIN transferencia_movimento ON transferencia_movimento.id_frete = frete_documento.id_frete
    LEFT JOIN transferencia ON transferencia.id = transferencia_movimento.id_transferencia
    WHERE transferencia_movimento.id_frete = '{$id_frete}' and transferencia.id_motorista = '{$id_motorista}' and transferencia.status != 10";
    ...
    transferencia.id
    FROM transferencia_movimento 
    LEFT JOIN transferencia ON transferencia.id = transferencia_movimento.id_transferencia
    WHERE transferencia_movimento.id_frete = '{$id_frete}' and transferencia.status != 10";
            $resultado_transferencia = mysqli_query($conn, $pesquisar_transferencia);
            $id_transferencia = '';
            while ($transferencia = mysqli_fetch_assoc($resultado_transferencia)) {
                $id_transferencia = $transferencia['id'];

                if (empty($id_transferencia)) {
                    throw new Exception('Erro campo id_transferencia, acione o suporte');
                }
...
tipo_acao_transferencia ='$tipo_acao' WHERE id_frete='$id_frete'";
            $sql->query($query_02);

            $query_02 = "UPDATE transferencia_movimento set 
status ='$tipo_acao' WHERE id_frete='$id_frete'";
            $sql->query($query_02);

            $pesquisar_tratativa = "SELECT status FROM transferencia_movimento WHERE id_transferencia = '" . $id_transferencia . "' and status = 12 ";
            ...
            $pesquisar_pendente = "SELECT status FROM transferencia_movimento WHERE id_transferencia = '" . $id_transferencia . "' and status = 6 ";
            ...
            $pesquisar_transito = "SELECT status FROM transferencia_movimento WHERE id_transferencia = '" . $id_transferencia . "' and status = 4 ";
            ...
            $pesquisar_aberto = "SELECT status FROM transferencia_movimento WHERE id_transferencia = '" . $id_transferencia . "' and status = 1 ";
            $resultado_aberto = mysqli_query($conn, $pesquisar_aberto);
            $aberto = mysqli_fetch_assoc($resultado_aberto);
            $aberto = $aberto['status'];

            $pesquisar_entregue = "SELECT status FROM transferencia_movimento WHERE id_transferencia = '" . $id_transferencia . "' and status = 9 ";
            ...
            $pesquisar_finalizado = "SELECT status FROM transferencia_movimento WHERE id_transferencia = '" . $id_transferencia . "' and status = 8 ";
            ...
            if ($tratativa != '') {
                ...
                $query_04 = "UPDATE transferencia set 
	status ='$tratativa' WHERE id='$id_transferencia'";
                $sql->query($query_04);

            } elseif ($pendente != '') {

                $query_04 = "UPDATE transferencia set 
	status ='$pendente' WHERE id='$id_transferencia'";
                $sql->query($query_04);

            } elseif ($transito != '') {

                $query_04 = "UPDATE transferencia set 
	status ='$transito' WHERE id='$id_transferencia'";
                $sql->query($query_04);

            } elseif ($aberto != '') {

                $query_04 = "UPDATE transferencia set 
	status ='$aberto' WHERE id='$id_transferencia'";
                $sql->query($query_04);

            } elseif ($entregue != '') {

                $query_04 = "UPDATE transferencia set 
	status ='$entregue' WHERE id='$id_transferencia'";
                $sql->query($query_04);

            } elseif ($finalizado != '') {

                $query_04 = "UPDATE transferencia set 
	status ='$finalizado' WHERE id='$id_transferencia'";
                $sql->query($query_04);
    ...
    $url = SITE_URL . 'administrativo.php?link=4041&transferencia=' . $id_manifesto;
